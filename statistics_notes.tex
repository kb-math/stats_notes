...
\begin{lemma} If $T:\cX \to \cT$ is a statistic and $\mu_{\Theta}$ is a prior on $\Theta$. Then for $A \subset \cX$ we have that $$P(X \in A ~|~ TX=Tx, \Theta = \theta) = P_{\theta}(A ~|~ T =Tx) \quad \text{for $P$-almost all $(x,\theta) \in \cX \times \Theta$}.$$
...
TODO: Show that the integrand on the Right hand side is measurable.
...
\begin{align*} \int_{T^{-1}C \times B} P_{\theta}(A | T=Tx) dP_{\theta}(x) d\mu_{\Theta}(\theta) &= \int_B P_{\theta}(A \cap T^{-1}C) d\mu_{\Theta}(\theta) \\
