...
For all Borel $B \subset X$ we have $$x \mapsto \int_{X} \mu_x(B) d\mu_x(u)$$ is $\cA$ measurable on $X'$ and for all $A \in \cA$ we have that $$\int_{A} \mu_x(B) d\mu(x) = \mu(A \cap B).$$
...
Independence does not since if we take $A = [0,1], B = [1,2]$ then $$\mu(X^{-1}A \times Y^{-1}B) = \mu([0,1]^2 \cap [1,2]^2) = 0$$ but $$\mu(X^{-1}A) = \mu([0,1]^2) = \frac{1}{2}$$ and $$\mu(Y^{-1}B) = \mu([1,2]^2) = \frac{1}{2}.$$
