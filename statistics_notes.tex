...
\begin{lemma} We have that $X$ and $Y$ are conditionally independent given $T$ if and only if for all $A \subset \cX$ we have that $$\mu(X^{-1}A | Y=y, T=t) = \mu(X^{-1}|T=t).$$

\end{lemma}

\begin{proof} First assume $X$ and $Y$ are conditionally independent. Thus we must show that $$\int_{U}\mu(A|T=t) = \mu(A \cap S^{-1}U)$$ for all $U \subset \cX \times \cT$ and $A \in \cA_X$. By the monotone class lemma, it is enough to show it for $U = B \times C$. We have 
...
Now we prove the converse. Thus we 
