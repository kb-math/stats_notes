...
\begin{thm} Assume the nice condition of ??. If $T:\cX \to \cT$ is sufficient in the Bayesian sense, then it is also sufficient in the classical sense.
...
\begin{corol} Assuming the nice condition of ??. We have that $\cT:\cX \to \cT$ is sufficient (in either classical of Bayesian sense) if and only if the Bayesian sufficiency condition is satisfied for all discrete prior distributions $\mu_{\Theta}$ on $\Theta$.
