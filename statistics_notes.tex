...
\begin{prop} Suppose that $d:\cX \to \bR$ is a UMVU esimator of $c:\Theta \to \bR$. Then if $T$ is a sufficient statistic, then $d \in L^2(\cX, \cB_T, P_{\theta})$ for all $\theta \in \Theta$. In other words, $d$ is $\cB_T$ almost measurable and so $d(x) = \hat{d}(x)$ for $P_{\theta}$-almost all $x \in \cX$. \end{prop}
...
\begin{mydef}[Complete sufficient statistic] We say that $T:\cX \to \cT$ is a \texit{complete sufficient statistic} if $T$ is complete and whenever a measurable $g:\cT \to \bR$ satisfies that $$E_{\theta}|g \circ T| < \infty \text{ and } E_{\theta}(g \circ T) = 0 \quad\text{for all } \theta \in \Theta$$ then we must have that $$g(Tx) = 0 \quad\text{for  $P_{\theta}$-almost all } x \in \cX.$$

\end{mydef}

Here is a geometric way of viewing completeness (at least for square measurable $g$): It says that if $g \circ T \in L^2(\cX, \cB_T, P_{\theta})$ is orthogonal to the space of constant functions then $g \circ T$ must be the zero vector in $L^2(\cX, \cB_T, P_{\theta})$.
