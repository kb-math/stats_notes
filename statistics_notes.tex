...
\begin{mydef}[Complete sufficient statistic] We say that $T:\cX \to \cT$ is a \textit{complete sufficient statistic} if $T$ is complete and whenever a measurable $g:\cT \to \bR$ satisfies that $$E_{\theta}|g \circ T| < \infty \text{ and } E_{\theta}(g \circ T) = 0 \quad\text{for all } \theta \in \Theta$$ then we must have that $$g(Tx) = 0 \quad\text{for  $P_{\theta}$-almost all } x \in \cX.$$
