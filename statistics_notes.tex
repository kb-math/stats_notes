...
and $t \mapsto r(B,t)$ is Borel on $\cT$.
...
Thus we may take $$r(\{x_0\},t) = \mathds{1}(Tx_0 = t) \binom{n}{t}^{-1}$$ and extend $r$ to $\cB \times \bZ$ by additivity. Thus this $T$ is indeed sufficient in the classical sense. 
...
\begin{proof} We first show the \textit{Moreover} claim. By the definition of sufficiency we have that $$P_{\theta}(A ~|~ T=t) = r(A,t)$$ for some measurable $t \mapsto r(A,t)$ and so indeed the Borel condition of the previous Lemma is satisfied. By the previous Lemma the left hand side of (\ref{P given T and theta is P_theta given T}) is equal to $r(A,Tx)$.  So now it remains to show that $r(A,Tx) = P(A \times \Theta ~|~ TX=Tx)$ for $P$-almost all $(x,\theta)$. Thus we must show that for $C \subset \cT$ we have that 
\begin{align}\label{eq: r(A,Tx) and P_X} \int_{T^{-1}C \times \Theta} r(A,Tx) dP(x,\theta) = \int_{T^{-1}C \times \Theta}  P(A \times \Theta ~|~ TX=Tx) dP(x,\theta).\end{align}
...
Recall that $d:\cX \to \cA$ is the decision function and $L:\Theta \times \cA \to [0,\infty)$ denotes the loss function. The risk is then $$R(\theta, d) = \int_{\cX} L(\theta, d(x)) dP_{\theta}(x).$$ Recall (see Theorem~\ref{thm: bayesian implies classical sufficient}) that if $T:\cX \to \cT$ is a sufficient statistic then there are families of measures $\nu^*_t$, for $t \in \cT$, such that $$P_{\theta}(B~|~T=t) = \nu^*_t(B) \quad \text{for all } B \subset \cX.$$

\begin{thm}[Rao-Blackwell theorem] Suppose that $\cA = \bR^d$ and that for each fixed $\theta$, the map $a \mapsto L(\theta, a)$ is convex. Suppose also that $x \mapsto d(x)$ is an $L^1$ function for each $P_{\theta}$, in other words $$\int_{\cX} \|d(x) \| dP_{\theta} < \infty.$$ Suppose that $T:\cX \to \cT$ is a sufficient statistic for the family of $P_{\theta}$. Then there is a $\cT$-measurable $\cX_d \subset \cX$ such that $P_{\theta}(\cX_d) = 1$ for all $\theta \in \Theta$ and a well-defined decision rule $\hat{d}:\cX_0 \to \bR^d$ given by \begin{align}\label{eqn: Rao-Blackwell estimator} \hat{d}(x) = E_{\theta}(d |T=Tx) = \int_{\cX} d(x') d\nu^*_{Tx}(x')\end{align} which satisfies that $$R(\theta, \hat{d}) \leq R(\theta, d).$$ Well-defined here means that integrals are absolutely convergent for all $x \in \cX_d$.
 
\end{thm}

Note that we can extend $\hat{d}$ to all of $\cX$ and the risk functions will not change.

\begin{proof} Note that as $d$ is $L^1$ with respect to $P_{\theta}$, we must have that it is $L^1$ with respect to $P_{\theta}( \cdot ~|~ T=Tx) = \nu^*_t(\cdot)$ for $P_{\theta}$-almost all $x$. Thus we can let $\cX_d$ be the set of all $x$ so that the integral converges absolutely (it does not depend on $\theta$). Thus the expression for $\hat{d}(x)$ is well defined. By Jensen's theorem, we have that 
$$L(\theta, \hat{d}(x)) = L\left(\theta, \int_{\cX} d(x') d\nu^*_{Tx}(x') \right) \leq \int_{\cX} L(\theta, d(x')) d\nu^*_{Tx}(x')  $$
...
Now by applying the Rao-Blackwell theorem to $d_2$, we have that $\hat{d_2}$ is an even better unbiased estimator for $c(\theta)$, and thus $$E_{\theta_0} |\hat{d_2} - c(\theta)|^2 < E_{\theta_0} |\hat{d} - c(\theta)|^2.$$ T
his implies that if we define $g:\cT \to \bR$ by\footnote{this is well defined as $\hat{d}, \hat{d_2}$ are functions of $T$ by construction in the Rao-Blackwell theorem above.} $g(Tx) = \hat{d}(x) - \hat{d_2}(x)$, then we must have that $g \circ T$ is not the zero-function in $L^2(\cX, \cB_T,P_{\theta_0})$. 
