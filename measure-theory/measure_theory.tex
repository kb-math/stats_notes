...
By induction, we can see that if $t_1, \ldots, t_n \in [0,1]$ is such that $t_1 + \cdots + t_n = 1$ then $$\sum_{i=1}^n t_i u_i \in U$$ for all $u_1, \ldots, u_n \in U$. Moreover, if $\varphi:U \to \bR$ is convex then $$\varphi \left(\sum_{i=1}^n t_i u_i \right) \leq \sum_{i=1}^n t_i f(u_i). $$
\end{proof}
...
&\leq t\varphi(x_0) + MR^{-1}\|u - x_0\|. \end{align*}
