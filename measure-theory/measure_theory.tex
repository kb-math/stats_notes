...
\begin{proof} We prove this by induction on $d$. The base case $d=0$ is trivial (where $U = \{0 \} = \bR^{0}$ is a zero-dimensional vector space). Now fix $x_0 \in U$. Let $U' \subset U$ be a cube contained in $U$ such that $x_0$ is the center of $U'$. Let $B$ be the boundary of this cube $U$. Observe that by the induction hypothesis, $\varphi$ is continuous on $B$ (as $B$ is made up of finitely many $d-1$-dimensional faces) and thus is bounded on $B$, say $|\varphi(b)| \leq M$ for all $b \in B$. Observe also that any $u \in U'$ may be written as $u = tx_0 + (1-t)b$ for some $b \in B$ (to see this, note that any ray starting at $x_0$ that goes through $u$ must intersect $B$ as some point $b$). 
...
&\leq \varphi(x_0) + R^{-1}(|\varphi(x_0)| + M)\|u-x_0\|. \end{align*}
 Thus $$\limsup_{u \to x_0} \varphi(u) \leq \varphi(x_0).$$ Now suppose for contradiction, that there is some $u_1,u_2, \ldots \in U$ such that $u_i \to x_0$ but $\varphi(u_i) < \varphi(x_0) - \epsilon$ for some fixed $\epsilon > 0$. Now observe that $$x_0 = t_iu_i + (1-t_i)b_i$$ for some $t_i \in [0,1]$ and $b_i \in B$ as we can see by considering a ray that starts at $u_i$ and passes through $x_0$ and then letting $b_i \in B$ be a point on this ray. We now have that $$\varphi(x_0) \leq t_i \varphi(u_i) + (1-t_i)\varphi(b_i) \leq t_i (\varphi(x_0) - \epsilon) + (1-t_i)M.$$ Now observe that $t_i \to 1$ as $u_i \to x_0$ since $$\|x_0 - u_i\| = \|(t_i-1)u_i + (1-t_i)b_i \| = (1-t_i)\|b_i - u_i\|.$$ Thus this upper bound converges to $\varphi(x_0) - \epsilon$, which is a contradiction. \end{proof}
...
&\leq t\sum_{i=1}^{n-1}t^{-1}t_i \varphi(u_i) + t_n \varphi(u_n) \\
...
\begin{proof} Note that if $\mu$ is supported on a finite set $\{u_1, \ldots, u_n\}$ with $\mu(\{u_i\}) = p_i$, then by the inequality above we have that $$ \varphi\left(\int_K x d\mu(x)\right)  = \varphi\left( \sum_{i=1}^n p_i u_i \right) \leq \sum_{i=1}^n p_i\varphi(u_i) = \int_K \varphi(x) d\mu(x) .$$ Thus the result holds for finite measures. For arbitrary probability measures on $K$, we use the proposition below and the continuity of $\phi$ and $x \mapsto x$.

\end{proof}

\begin{prop} Let $\mu$ be a measure on a compact set $[-R,R]^d \subset \bR^d$. For each positive integer $n$, let $\cC_n$ be a partition of $[-R,R]^d$ into finitely many cubes, i.e., $$[-R,R]^d = \bigcup_{C \in \cC_n} C$$ such that each $C \in \cC^n$ has diameter at most $1/n$. Let $C_n \subset [-R,R]^d$ be a collection of points consisting of exactly one element from $\cC_n$, thus we have a bijection $\iota:C_n \to \cC_n$ where $c \in \iota(c)$. Now define a probability measure on $C_n$ given by $\mu_n(\{c\}) = \mu(\iota(c))$ for $c \in C_n$. Then $$\int_{\bR^d} f d\mu_n \to \int_{\bR^d} f d\mu$$ for all continuous function $f:[-R,R]^d \to \bR.$

\end{prop}

We now obtain immediately Jensen's inequality for integrable random variables.

\begin{thm}[Jensen's inequality] Let $\mu$ be a probability measure  on $\bR^d$ such that $$\int_{\bR^d} \|x \| d\mu(x) < \infty$$ and suppose $\varphi:\bR^d \to \bR$ is convex. Then $$\varphi\left(\int_{\bR^d} x d\mu(x)\right) \leq \liminf_{R \to \infty} \int_{[-R,R]^d} \varphi(x) d\mu(x). $$
...
$$\varphi \left(\int_{\Omega} f(\omega) d\nu(\omega) \right) \leq \liminf_{R \to \infty} \int_{f^{-1}([-R,R]^d)} \varphi(f(\omega)) d\nu(\omega).$$ 
